...
namespace net
{
    ...
    {
        return boost::dynamic_pointer_cast<TcpSocket>(aSocket);
    ...
    {
        return boost::dynamic_pointer_cast<UdpSocket>(aSocket);
        ...
        )
    {
        ...
        while( !( endOfLineFound = extractLineFromInternalBuffer( aLine ) ) )
        {
            ...
            // Resever some space for reading a chunk.
            size_t leftover = iInternalBuffer.size();
            iInternalBuffer.resize( READ_LINE_CHUNCK_SIZE );

            int recvBytes = posixReceive
                (
                &iInternalBuffer[leftover],
                READ_LINE_CHUNCK_SIZE - leftover,
                100 // use 10ms as the timeout granuality
                );

            if( recvBytes <= 0 )
            {
                iInternalBuffer.resize(leftover);
                continue;
            }

            trace("Receive %d bytes!\n", recvBytes);
            iInternalBuffer.resize( recvBytes + leftover );
        }
